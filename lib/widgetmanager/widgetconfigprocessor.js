  ...
  WidgetConfigProcessor.prototype.processConfigurationDocument = function (res, userAgentLocales, processingResult, processListener) {
    ...
    if (configBuffer) {
      ...
      var startDocument = function (elt) {
        ...
        if ('autostart' in attrs) {
          /*
           * Added by Toby - for service widget auto-start support.
           */
          var autostart = processNonNegativeIntAttr(attrs.autostart);

          Logger.logAction(Logger.LOG_MINOR, "autostart", "setting auto-start");
          widgetConfig.autostart = autostart !== 0;
        }
      };
